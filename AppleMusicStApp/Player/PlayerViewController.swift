...
class PlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        UIApplication.shared    // 현재 앱에대한 SingleTon
        updatePlayButton()
...
//        CMTime(seconds: 1, preferredTimescale: 10) -> 0.1초
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        updateTintColor()   // dark Mode에서 색상을 업데이트
        updateTrackInfo()
    }
    ...
    @IBAction func seek(_ sender: UISlider) {
        ...
        let position = Double(sender.value) // 0.....1 > 0.5 timeSlider의 현재 값
        let seconds = position * currentItem.duration.seconds
        
        let time = CMTime(seconds: seconds, preferredTimescale: 100)    // 두자릿수의 Double로 만들기 위해 preferredTimescale은 100으로 해준다.
